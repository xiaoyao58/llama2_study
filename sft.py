...
if __name__=="__main__":
   ...
   # various inits, derived attributes, I/O setup
    ...
    # 加载预训练模型
    state_dict = torch.load('out/cauchy_pretrain_v21/epoch_19.pth')
    ...
    # 过滤掉尺寸不匹配的参数，使用原始模型的参数
    filtered_state_dict = {}
    for k, v in new_state_dict.items():
        # 如果是激活函数中的参数并且尺寸不匹配，则跳过
        if 'feed_forward.activation' in k and k in original_state_dict:
            if v.size() != original_state_dict[k].size():
                print(f"Skipping parameter {k} due to size mismatch: {v.size()} vs {original_state_dict[k].size()}")
                continue
                
        if k in original_state_dict and v.size() == original_state_dict[k].size():
            filtered_state_dict[k] = v
        elif k in original_state_dict:
            print(f"Skipping parameter {k} due to size mismatch: {v.size()} vs {original_state_dict[k].size()}")
    ...
    # 记录日志
    logger.info(f"Loaded {len(filtered_state_dict)} parameters from pretrained model")
